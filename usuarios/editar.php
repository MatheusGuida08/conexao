...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if(!empty($_POST['senha'])){
        ...
        $param = [':nome'=>$nome, ':email'=>$email, ':senha'=>$senha, ':tipo'=>$tipo, ':id'=>$id];
    } else {
        $sql = "UPDATE usuarios SET nome=:nome, email=:email, tipo=:tipo WHERE id=:id";
        $param = [':nome'=>$nome, ':email'=>$email, ':tipo'=>$tipo, ':id'=>$id];
    }
    try {
        $stmt = $pdo->prepare($sql);
        $stmt->execute($param);
        ...
        $mensagem = '<div class="error">Erro ao atualizar usuário: ' . $e->getMessage() . '</div>';
    }
}
